...
export interface CustomReCaptchaCopyProps {
...
const CustomReCaptchaCopy: React.FC<CustomReCaptchaCopyProps> = ({
  onChange,
  error = false,
  disabled = false,
      ...
      setTimeout(() => {
        if (windowWithRecaptcha.grecaptcha && windowWithRecaptcha.grecaptcha.render) {
          try {
            ...
            console.error('Error rendering reCAPTCHA (Copy):', error);
          }
        }
      }, 100);
    }
    ...
    <div className={`recaptcha-container ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      {error && (
        <div className="mt-1 text-xs text-red-500 flex items-start">
          ...
          <span>Please complete the reCAPTCHA verification (Copy Form)</span>
        </div>
      )}
    </div>
  );
...
export default CustomReCaptchaCopy; 