...
const EmptyState: React.FC = () => (
  ...
  >
    ...
    <p className="text-sm text-gray-500 dark:text-gray-400">
      Check back later for new recorded content.
    </p>
...
const RecordedClassCard: React.FC<{ classItem: RecordedClass }> = ({ classItem }) => (
  <motion.div
  ...
  >
    ...
    <div className="absolute top-4 left-4 z-10">
      <motion.div
        ...
        className="flex items-center gap-2 bg-blue-500/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-sm font-medium"
      >
    ...
    <div className="rounded-xl overflow-hidden mb-4 relative group">
      <Image
        ...
        alt={classItem.meet_title || "Recorded Session"}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <div className="absolute inset-0 flex items-center justify-center">
          <LucidePlayCircle className="w-12 h-12 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        </div>
      </div>
    </div>
    ...
    <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3">
      {classItem.meet_title || "Untitled Session"}
    </h3>
    ...
    <div className="space-y-3 mb-6">
      ...
      {classItem.courseDetails?.duration && (
        <div className="flex items-center text-gray-600 dark:text-gray-300">
          <LucideVideo className="w-5 h-5 mr-2 text-blue-500 dark:text-blue-400" />
          <span className="text-sm">
            Duration: {classItem.courseDetails.duration}
          </span>
        </div>
      )}
    </div>
    ...
    <div className="mt-auto space-y-3">
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-semibold rounded-xl transition-all duration-300"
      >
...
const Access_Recorded_Sessions_Instructor: React.FC = () => {
      ...
      const fetchRecordedClasses = () => {
        getQuery({
          ...
          onSuccess: (res: ApiResponse) => {
            const recordedSessions = res?.meetings?.filter(
              (classItem) => 
                classItem.meeting_tag === "recorded" && 
                classItem.courseDetails?.course_tag === "Pre-Recorded"
            ) || [];
            setClasses(recordedSessions);
            setErrorMessage("");
          },
          onFail: (err) => {
            ...
            setErrorMessage(
              ...
              "Failed to fetch recorded sessions. Please try again."
            );
          },
        });
      };
        ...
        >
          {classes.length > 0 ? (
            classes.map((classItem) => (
              <RecordedClassCard key={classItem._id} classItem={classItem} />
            ))
          ) : (
