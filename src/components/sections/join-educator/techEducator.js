...
const TechEducator = () => {
  ...
  return (
    <section className="py-10 w-full dark:bg-black bg-white flex justify-center items-center ">
      <div className="lg:w-[80%] w-[92%] text-center">
        <div className="lg:px-30 px-2 ">
          <h2 className="text-[26px] leading-9 font-bold dark:text-white text-[#252525] ">
        ...
        <div className="mt-10 w-full  flex flex-wrap justify-center gap-6">
          {criteriaData.map((item, index) => (
            <div
              key={index}
              className="w-full sm:w-[90%] md:w-[45%] lg:w-[23%] p-6 bg-white shadow-box-custom rounded-2xl border dark:bg-black dark:border-whitegrey3 border-[#0000004D] mb-6 hover:shadow-lg hover:scale-105 transition-transform duration-300 ease-in-out"
            >
              <h3 className="text-xl font-semibold dark:text-white text-gray-900 mb-4">
                {item.title}
              </h3>
              <p className="text-gray-700 dark:text-whitegrey3">
                {item.description}
              </p>
            </div>
          ))}
