...
const HiringProcess = () => {
  ...
  return (
    <div
      id="enroll-section"
      className="w-full px-4 py-12 bg-white dark:bg-screen-dark flex justify-center items-center"
    >
      <div className="lg:w-[80%] w-[96%]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 dark:bg-inherit lg:grid-cols-3 gap-8">
          {processSteps.map((step, index) => (
            <div
              key={index}
              className="border-2 dark:border-gray600 p-4 rounded-lg shadow-lg flex flex-col justify-between text-center relative dark:bg-screen-dark bg-white self-stretch transition-transform transform hover:scale-105 hover:shadow-xl"
            >
              <Image src={step.icon} alt="img" className="mx-auto h-16 mb-4" />
              <h3 className="text-[22px] leading-6 font-bold mb-2 text-[#020101] dark:text-gray50 mt-2">
                {step.title}
              </h3>
              ...
              {step.arrow && (
                <Image
                  src={Arrow}
                  alt="Arrow"
                  className="absolute right-0 top-1/2 transform translate-x-full -translate-y-1/2 h-20 w-20 dark:hidden bg-gray-100 hidden lg:block"
                />
              )}
            </div>
          ))}
