      ...
      <div className="w-[92%] lg:w-[80%]">
        ...
        <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-x-5 lg:gap-y-10 gap-y-5">
          {educationFeature.map((feature, index) => (
            <div
            key={index}
            className=" px-2 py-2 text-center bg-white rounded-3xl border border-[#0000004D] shadow-card-custom w-full transition-transform duration-300 ease-in-out hover:shadow-lg hover:scale-105 "
          >
            <Image src={feature.icon} alt="img" className="mx-auto h-16 mb-2" />
            <h3 className="text-[15px] leading-[22.5px] font-semibold text-[#252525]  font-Poppins mb-1">
              {feature.title}
            </h3>
