          ...
          <div className={`flex-1 flex flex-col justify-center transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
            ...
            <div className="mx-auto mb-4 glass-container rounded-2xl p-4 shadow-2xl max-w-sm sm:max-w-md md:max-w-lg">
              <div className="text-center">
                {/* ISO Badge */}
                <div className={`inline-flex items-center px-2 py-1 glass-stats rounded-full text-xs font-medium mb-2 opacity-95 ${isDark ? 'text-blue-300' : 'text-blue-700'}`}>
                  <Image src={Iso} alt="ISO Certification" width={12} height={12} className="w-3 h-3 mr-1" />
                  ISO 9001:2015 CERTIFIED
                </div>
                
                {/* Corporate Badge */}
                <div className={`inline-flex items-center px-2 py-1 glass-stats rounded-full text-xs font-medium mb-3 opacity-95 ml-2 ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`}>
                  <Sparkles size={10} className="mr-1" />
                  Corporate Training Excellence
                </div>
                
                {/* Main Heading - Proper Caps */}
                <h1 className={`text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 leading-tight tracking-tight ${isDark ? 'text-white' : 'text-gray-900'} text-shadow-light`}>
                ...
                <div className="mt-3 sm:mt-4 mb-3">
                  ...
                  <button 
                    ...
                    className={`inline-flex items-center justify-center py-2.5 px-4 font-medium rounded-xl transition-all duration-300 hover:scale-105 text-sm ml-2 ${
                      isDark 
                        ? 'glass-stats text-white hover:bg-white/10' 
                        : 'bg-gray-100/80 backdrop-blur-md text-gray-700 hover:bg-gray-200/80'
                    }`}
                  >
                    Contact Us
                  </button>
