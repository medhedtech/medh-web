...
const PlacementFAQ: React.FC = () => {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "Enrollment Process":
        return <HelpCircle className="w-8 h-8 text-green-600" />;
      case "Job Guarantee Details":
        return <CreditCard className="w-8 h-8 text-purple-600" />;
      case "Internship Program":
        return <Briefcase className="w-8 h-8 text-orange-600" />;
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-16">
          {categories.map((category) => (
            <div key={category} className="group">
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/50 p-8 md:p-12 hover:shadow-3xl transition-all duration-500 hover:scale-[1.02]">
                {/* Category Header */}
                <div className="flex items-center justify-center mb-10">
                  <div className="flex items-center space-x-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 px-6 py-3 rounded-full border border-blue-200/50 dark:border-blue-700/50">
                  ...
                  {faqData
                    ...
                    .map((item, index) => {
                      ...
                      return (
                        <div
                          key={globalIndex}
                          className="group/faq bg-gradient-to-r from-gray-50/80 to-blue-50/80 dark:from-gray-700/80 dark:to-blue-900/20 rounded-2xl border border-gray-200/50 dark:border-gray-600/50 overflow-hidden hover:shadow-lg transition-all duration-300 hover:border-blue-300/50 dark:hover:border-blue-500/50"
                        >
                          <button
                            onClick={() => toggleItem(globalIndex)}
                            className="w-full px-8 py-6 text-left bg-transparent hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-indigo-50/50 dark:hover:from-blue-900/20 dark:hover:to-indigo-900/20 transition-all duration-300 flex items-center justify-between group/button"
                          >
                            <div className="flex items-start space-x-4 flex-1">
                              ...
                              <span className="font-semibold text-gray-900 dark:text-white text-lg leading-relaxed group-hover/button:text-blue-700 dark:group-hover/button:text-blue-300 transition-colors duration-200">
                                {item.question}
                              </span>
                            </div>
                            <div className="flex-shrink-0 ml-4">
                              <div className={`w-10 h-10 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? 'rotate-180' : ''}`}>
                                {isOpen ? (
                          ...
                          <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="px-8 pb-6 bg-white/60 dark:bg-gray-800/60 border-t border-gray-200/50 dark:border-gray-600/50">
                              <div className="pt-6">
        ...
        <div className="mt-20 text-center">
          <div className="bg-gradient-to-r from-blue-600/10 via-indigo-600/10 to-purple-600/10 dark:from-blue-500/20 dark:via-indigo-500/20 dark:to-purple-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-blue-200/50 dark:border-blue-600/50 shadow-2xl">
            <div className="flex items-center justify-center mb-4">
              <div className="w-12 h-12 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full flex items-center justify-center">
                <HelpCircle className="w-6 h-6 text-white" />
              </div>
            </div>
            <p className="text-blue-800 dark:text-blue-200 font-semibold text-lg md:text-xl">
