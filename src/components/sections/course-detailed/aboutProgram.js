...
export default function CombinedProgram({ courseId }) {
  ...
  const fetchCourseDetails = async (id) => {
    try {
      await getQuery({
        ...
        onSuccess: (data) => {
          setCourseDetails1(data);
          // Check if curriculum exists, if not set dummy data
          setCurriculum(data.curriculum || getDummyCurriculum());
        },
        onFail: (err) => {
          console.error("Error fetching course details:", err);
        },
  ...
  const getDummyCurriculum = () => {
    // Return dummy curriculum data when no curriculum is available
    return [
      {
        weekTitle: "Week 1 - Introduction to Manual Testing",
        weekDescription:
          "Overview of manual testing concepts, testing types, and life cycle.",
      },
      {
        weekTitle: "Week 2 - Test Case Creation",
        weekDescription:
          "Learn how to create effective test cases, including writing clear and detailed steps.",
      },
      {
        weekTitle: "Week 3 - Bug Reporting and Management",
        weekDescription:
          "Learn how to report bugs using different tools and manage them through the lifecycle.",
      },
    ];
  };
              ...
              {curriculum.map((item, index) => (
                ...
                >
                  ...
                  >
                    ...
                    {/* Left aligned + or - */}
                    <span className="text-[1rem]">{item?.weekTitle}</span>
                  </button>
                  {openAccordions === index && (
                    <div className="py-3 pl-7 dark:bg-[#050622] bg-gray-50 text-[15px] text-gray-600">
                      <p>{item?.weekDescription}</p>
                    </div>
                  )}
                </div>
              ))}
