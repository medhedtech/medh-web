...
const StudentEnrolledCourses: React.FC = () => {
  ...
  const fetchEnrolledCourses = (studentId: string) => {
    getQuery({
      url: `${apiUrls?.EnrollCourse?.getEnrolledCoursesByStudentId}/${studentId}`,
      onSuccess: (data) => {
        const allCourses = data.map((enrollment: any) => enrollment.course_id);
        setEnrolledCourses(allCourses);
        const liveCoursesFiltered = allCourses.filter(
          (course: Course) => course.course_category === "Live Courses"
        );
  ...
  const fetchSelfPacedCourses = (studentId: string) => {
    getQuery({
      ...
      onSuccess: (response) => {
        ...
        const selfPacedCourses = enrolledCourses
          .filter((course: any) => course.is_self_paced === true)
          .map((course: any) => ({
            ...
            category:
              response?.data
                ...
                ?.category_ids.map((cat: any) => cat.category_name)
                .join(", ") || "N/A",
